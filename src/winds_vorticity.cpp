...
public:
    ...
    virtual void Execute(vtkObject *caller, unsigned long, void*) {
        ...
        if (state == 0) {
            // disable time _30
            renderer->RemoveActor(sgridActors[2]);
            ...
            // disable time _10
            renderer->RemoveActor(sgridActors[0]);
            renderer->RemoveActor2D(texts[0]);

            // enable time _20
            renderer->AddActor(sgridActors[1]);
            renderer->AddActor2D(texts[1]);
        } else {
            // disable time _20
            renderer->RemoveActor(sgridActors[1]);
            renderer->RemoveActor2D(texts[1]);

            // enable time _30
            renderer->AddActor(sgridActors[2]);
            renderer->AddActor2D(texts[2]);
        }

    }
    vtkButtonCallback() : sgridActors(std::vector<vtkSmartPointer<vtkActor>>(0)), renderer(0), actor(0), renderWindow(0), renderWindowInteractor(0), styleTrackball(0), texts(std::vector<vtkSmartPointer<vtkTextActor>>(0)){}
    std::vector<vtkSmartPointer<vtkActor>> sgridActors;
...
int main(int, char *[]) {
    ...
    for (int i = 0 ; i < 3 ; i++) {
        ...
        hedgehog->SetInputConnection(readers[i]->GetOutputPort());
        hedgehog->GetInputPortInformation(0);
        hedgehog->Update();
...
//    renderer->SetBackground(1, 1, 1);
    ...
    callbackButton->sgridActors = sgridActors;
