...
int main(int, char *[]) {
    cout << "This program will display the cloud pressure data" << endl << std::flush;
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < 3; i++) {
      ...
      // TODO: check if name is correct (via paraview what exactly we need to set as active)
      // image->GetPointData()->SetActiveVectors(image->GetPointData()->GetScalars()->GetName().c_str());
      image->GetPointData()->SetActiveVectors("pres");
      images[i] = image;
    }
    cerr << " done" << endl;
    ...
    for (int i = 0; i < 3; i++) {
      ...
      glyphFilter->SetInputData(images[i]);
      glyphFilter->Update();
    ...
    for (int i = 0 ; i < 3 ; i++) {
      ...
      imageMapper->SetInputData(images[i]);
