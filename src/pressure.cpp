...
int main(int, char *[]) {
    ...
    for (int i = 0 ; i < 3 ; i++) {
        ...
        image->GetPointData()->SetActiveScalars("pres");
        ...
        float valuesRange[2];
        /// WARNING: Following only works for .pres files
        vtkFloatArray::SafeDownCast(image->GetPointData()->GetAbstractArray("pres"))->GetValueRange(valuesRange);
