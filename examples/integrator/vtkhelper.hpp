...
vtkSmartPointer<vtkActor> createLineActor(const std::vector<vec2f>& line, const vec3f& color)
{
	// create polyline vertices
	vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
	for (const vec2f& vertex : line)
		points->InsertNextPoint(vertex.x(), vertex.y(), 0);
